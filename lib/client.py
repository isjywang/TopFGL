...
class Client():
    ...
    def local_train(self, local_epoch):
        train_stats = train_FGL(self.model, self.dataLoader, self.optimizer, local_epoch, self.args.device, self.id)
...
class Client_fedtop():
    ...
    def local_train(self, local_epoch):
        train_stats = train_fedtop(self.model, self.topology_learner, self.dataLoader, self.optimizer, self.structual_optimizer, local_epoch, self.args.device, self.id)
...
def train_fedtop(model, top_learner, dataloaders, opt, opt_tl, local_epoch, device, client_id):
    acc_test = []
    for epoch in range(local_epoch):
        model.train()
        top_learner.train()
