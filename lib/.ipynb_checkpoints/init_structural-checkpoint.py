...
def compute_node2vec_embeddings(g, embedding_dim=8, walk_length=6, context_size=6,
    ...
    # print(f"Generated RWE tensor shape: {RWE.shape}")
...
def init_fedtop(g):
    ...
    # print("RWE init successfully finished!")
    ...
    # print("shape of topological embeddings:", random_emb.shape, degree_emb.shape, jaccard_emb.shape)
    g['stc_enc'] = torch.cat([random_emb, degree_emb, jaccard_emb], dim = 1)
    return g

        
# prepare for fedtop -- topological embedding init, run before the training.
...
#                 client_graph = init_fedtop(client_graph)
